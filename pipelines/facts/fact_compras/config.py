...
SELECT_ORIGEN = """
    ...
    SELECT
        ...
        CASE 
			--COMPRA RRHH, COMPRA ADMINISTRACION, SOL. RQ X STOCK
			WHEN ISNULL(CA20.PROCESOO, COMAPROREQCAB.PROCESOR) IN (14,15, 29, 36, 38, 39)
			AND ISNULL(DE20.Estado, Niveles.Estado) IN (25,26)
			THEN 23 --RQ APROBADO
				
			--COMPRA LIMA - SOLO LOS QUE PASAN POR COTIZACION
			WHEN ISNULL(CA20.PROCESOO, COMAPROREQCAB.PROCESOR) IN (4, 18, 20)
			 AND ISNULL(DE20.Estado, Niveles.Estado) IN (23,26)
			 AND COMAPROREQDET.NIVELAPROB = 3
			 AND COMAPROREQDET.OCOMPRA = 0
			THEN 78 --PEND. COTIZACION

			--COMPRA LIMA - SOLO LOS QUE PASAN POR ADMINISTRACION
			WHEN ISNULL(CA20.PROCESOO, COMAPROREQCAB.PROCESOR) IN (4, 18, 20)
			 AND ISNULL(DE20.Estado, Niveles.Estado) IN (23,26)
			 AND COMAPROREQDET.OCOMPRA = 0
			THEN 23 --PEND. APROBACION

			-- COMPRA LIMA - YA CUENTA CON ORDEN DE COMPRA
			WHEN ISNULL(CA20.PROCESOO, COMAPROREQCAB.PROCESOR) IN (4, 18, 20)
			 AND ISNULL(DE20.Estado, Niveles.Estado) IN (15,16)
			 AND COMAPROREQDET.OCOMPRA = 1
			THEN 18 --PEND. RECEPCION

			ELSE ISNULL(DE20.Estado, Niveles.Estado)
		END AS F_ESTADO_ITEM,

        
        CASE 
			WHEN  CA20.EstadoDoc = 16
			THEN 15 ELSE  CA20.EstadoDoc
		END AS F_ESTADO_OC,
        ...
        CASE WHEN COALESCE(DE20.afectoigv, COMAPROREQDET.afectoigv, 1) = 0 THEN ISNULL(NULLIF(tipo_igv_d20,0), 0.18) ELSE 0 END AS F_IGV
    FROM ALMPEDIDOCAB
    ...
    AND ALMPEDIDOCAB.FCHEDIT >= DATEFROMPARTS(YEAR(GETDATE()) - 2, 1, 1)
    AND ALMPEDIDOCAB.FCHEDIT <  DATEFROMPARTS(YEAR(GETDATE()) + 2, 1, 1)
    AND ISNULL(CA20.PROCESOO, COMAPROREQCAB.PROCESOR) IS NOT NULL
...
UPDATE_RECIBIDO_QUERY = """
	...
	WHERE CA20.Y20004 = 'P'
	AND CA20.EstadoDoc <> 3
	AND EXISTS (
		SELECT 1 FROM DE10 WHERE DE10.SEQLIQ = DE20.Y20005
	)
	AND ISNULL(CA20.fecha_emi_oc, CA20.Y20006) >= DATEFROMPARTS(YEAR(GETDATE()) - 1, 1, 1)
    AND ISNULL(CA20.fecha_emi_oc, CA20.Y20006) <  DATEFROMPARTS(YEAR(GETDATE()) + 1, 1, 1)
	AND CA20.EstadoDoc NOT IN (3, 17, 27, 31, 71)
	AND DE20.Estado <> 3;

	UPDATE DE20
	SET DE20.Estado =
		CASE
			WHEN ISNULL(DE20.RECIBIDO, 0) = 0 THEN 18
			WHEN ISNULL(DE20.RECIBIDO, 0) = DE20.Y20084 THEN 6
			WHEN ISNULL(DE20.RECIBIDO, 0) > 0 AND  ISNULL(DE20.RECIBIDO, 0) < DE20.Y20084 THEN 5
			ELSE DE20.Estado
		END
	FROM DE20
	INNER JOIN CA20 ON CA20.Y20005 = DE20.ID_OC
	WHERE CA20.Y20004 = 'P'
	...
	FROM ca20
	INNER JOIN (
		SELECT DISTINCT de20.id_oc FROM de20
		WHERE EXISTS (
			SELECT 1 FROM DE10 WHERE DE10.SEQLIQ = DE20.Y20005
		)
	) AS DET20 ON DET20.id_oc = ca20.Y20005
	WHERE CA20.Y20004 = 'P'
	AND CA20.EstadoDoc <> 3
	AND ISNULL(CA20.fecha_emi_oc, CA20.Y20006) >= DATEFROMPARTS(YEAR(GETDATE()) - 2, 1, 1)
    AND ISNULL(CA20.fecha_emi_oc, CA20.Y20006) <  DATEFROMPARTS(YEAR(GETDATE()) + 2, 1, 1)
	AND CA20.EstadoDoc NOT IN (3, 17, 27, 31, 71);
"""
...
#     CASE
#         -- HAY DEVOLUCIÓN
#         WHEN ISNULL(DE20.CantDevolucion, 0) > 0 THEN
#             CASE
#                 WHEN ISNULL(DE20.RECIBIDO, 0) = DE20.Y20084 THEN 6
#                 ELSE 5
#             END
...
# 	SELECT 
# 		CASE 
# 			WHEN COUNT(de20.id_oc) = 0 THEN ca20.EstadoDoc -- NO HAY DETALLES, MANTENER EL ESTADO ACTUAL
# 			WHEN COUNT(CASE WHEN de20.Estado = 5 THEN 1 END) > 0 THEN 5 -- SI HAY AL MENOS UN DETALLE CON ESTADO 5
# 			WHEN COUNT(CASE WHEN de20.Estado != 6 THEN 1 END) = 0 THEN 6 -- TODOS LOS DETALLES SON 6
# 			WHEN COUNT(CASE WHEN de20.Estado != 3 THEN 1 END) = 0 THEN 3 -- TODOS LOS DETALLES SON 3
# 			WHEN COUNT(CASE WHEN de20.Estado != 17 THEN 1 END) = 0 THEN 17 -- TODOS LOS DETALLES SON 17
# 			ELSE ca20.EstadoDoc -- ESTADOS MEZCLADOS → NO CAMBIAR
# 		END
# 	FROM de20
# 	WHERE de20.id_oc = CA20.Y20005
# )
